...
**Status — YOLO detector (30 Jul 2026):** MASC-only training completed the full 60/60 epochs (on teammate Salar's `salar` branch, merged into `noor`) — final precision 0.535, recall 0.447, mAP50 0.434, mAP50-95 0.322 on the MASC val split (see Supplementary Progress Report §15C.7 for full run results; supersedes the earlier 1-epoch interim figures). Best checkpoint exported (`models/yolo_ui_detector_best.pt`, 51.2 MB). **`src/yolo_ui_detector.py` now exists** (ported from Salar's branch 30 Jul — corrects the 29 Jul claim that it did not exist), exposing `detect_ui(image_path)`. **Rico holdout generalization eval done:** zero-shot mAP50 0.0258 → fine-tuned mAP50 0.2546 (§15C.9). Still **not wired into the audit pipeline** — deliberately deferred, as this remains a Stretch requirement. See SDS §3.6 for design detail.

**Status — Crop classifier / FR-CV.1–3, FR-CV.8–9 (30 Jul 2026):** MobileNetV3-Small trained for 20 epochs on MASC (25,202/5,558/5,541 train/val/test crops), best val macro-F1 0.265 at epoch 17. Test-set F1: R08 0.70, R17 0.47, R04 0.36; R09/R10/R28 have zero positive test examples in the current MASC corpus (no declared color/text-size signal — a data-coverage gap, not a code gap). **Rico holdout generalization eval done** (30 Jul): 9,343 crops from all 1,698 holdout screens, weighted F1 0.43 (vs. 0.63 on MASC test) — confirms real generalization, not pure memorization, with the expected accuracy drop on unseen apps (§15G.7). `src/crop_violation_classifier.py`'s `classify_crop()` is real, imports and runs — but nothing in `backend/` calls it yet (FR-CV.1's "supplementary signal" is trained and evaluated but not connected to the live pipeline, deliberately deferred as a Stretch requirement). See SDS §3.7 and Supplementary Progress Report §15G for full design and results.
...
| FR-CV.1–3, FR-CV.8–9 | Crop classifier (multi-label R09/R04/R17/R10/R28/R08) | Stretch | §4.4 | Noor | **Trained + Rico-evaluated, not wired** — MobileNetV3-Small, 20 epochs, best val macro-F1 0.265 (epoch 17); Rico holdout weighted F1 0.43; `classify_crop()` real and working; not called from `backend/` — deliberately deferred |
| FR-CV.4–7 | Screenshot-only YOLO UI-detector fallback | Stretch | §4.4 | Noor (+ Salar notebook/train/eval) | **Trained + Rico-evaluated, not wired** — full 60-epoch MASC training run (mAP50-95 0.322); `best.pt` exported (51.2 MB); Rico holdout mAP50 0.0258 → 0.2546; `src/yolo_ui_detector.py` **exists** (corrected 30 Jul); not called from `backend/` — deliberately deferred |
