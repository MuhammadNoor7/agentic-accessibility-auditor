...
| **Stretch** | R21–R30 rules; multi-label crop classifier (R09/R04/R17/R10/R28/R08, §4.4) — trained and wired into the backend pipeline; YOLO screenshot-only fallback detector — trained and wired into the backend pipeline; interactive click-to-highlight in HTML report; benchmark dataset export |
...
| F10 | **Crop-level pixel classifier** — Multi-label CV signal (R09/R04/R17/R10/R28/R08) supplementing the XML rule checker on rendered pixels | Stretch — **Done, wired into backend pipeline** |
...
| F13 | **Screenshot-only fallback detection (YOLO)** — Detect UI elements directly from the screenshot pixels when no (or a malformed) XML hierarchy is available | Stretch — **Done, wired into backend pipeline** |
...
| **LLM** | Multi-provider (`src/llm_providers.py`, `LLM_PROVIDER` env var: anthropic/openai/gemini/groq) via HTTPS; **deployed default: Groq** (`llama-3.3-70b-versatile`) — the only provider with a working free tier (see TBD-01, Appendix D); API key in environment variables only |
...
| C4 | Only open-source tools except the **LLM API** (team-provisioned; deployed default **Groq**, free tier — see TBD-01) |
...
| pytest suite | Salar + Ayesha + Noor | Parser, rules, agent, audit, report tests |
...
- A working LLM API key is available for the project duration — resolved as **Groq** (free tier), since Anthropic/OpenAI require paid billing and Gemini's free tier hit a quota bug (TBD-01, Appendix D)
...
| LLM API (Groq default) | Agentic explanation and fix suggestion layer |
...
| Crop classifier (`swin_tiny_patch4_window7_224`) + YOLO detector (Stretch) | Pixel-level rule confirmation + screenshot-only fallback; both trained and wired into the backend |
...
| LLM API (Groq default, `llama-3.3-70b-versatile`) | v1 | Agentic layer | REST HTTPS |
...
| Crop classifier (Stretch) | `swin_tiny_patch4_window7_224` | Pixel-level rule confirmation (R09/R04/R17/R10/R28/R08) | `models/crop_violation_classifier_best.pt` |
| YOLO UI detector (Stretch) | `yolo11s`/`yolo26n`-based, 60 epochs | Screenshot-only fallback detection | `models/yolo_ui_detector_best.pt` |
...
| FR-COM.4 | LLM API called **server-side only**; API key never exposed to client |
...
**Status — YOLO detector (05 Aug 2026):** MASC-only training completed the full 60/60 epochs (on teammate Salar's `salar` branch, merged into `noor`) — final precision 0.535, recall 0.447, mAP50 0.434, mAP50-95 0.322 on the MASC val split (see Supplementary Progress Report §15C.7 for full run results; supersedes the earlier 1-epoch interim figures). Best checkpoint exported (`models/yolo_ui_detector_best.pt`, 51.2 MB). `src/yolo_ui_detector.py` exposes `detect_ui(image_path)` plus a new `detections_to_components()` converter that maps raw detections into `components.json`-shaped entries tagged `inferred: true` (FR-CV.7). **Rico holdout generalization eval:** zero-shot mAP50 0.0258 → fine-tuned mAP50 0.2546 (§15C.9). **Now wired into the audit pipeline** (05 Aug 2026): `xml` is optional on `POST /api/v1/audit`; `_run_pipeline` falls back to `detect_ui()` when the paired XML is missing, malformed, or produces zero components — FR-CV.4 fully satisfied, no longer deferred. See SDS §3.6 for design detail.
...
| NFR-10 | Security | LLM API key **never** exposed to client browser | Must |
...
| FR-DK.1–3 | Docker Compose | Must | §4.7 | Salar/Noor | **Code fixes applied, live re-verification pending** (synced to `noor`; 05 Aug fixes by Noor: (1) `backend/requirements.txt` synced with torch/torchvision/timm/ultralytics — was missing, caused a backend container startup crash; (2) `backend/Dockerfile` now installs `libgl1`/`libglib2.0-0`/`libsm6`/`libxext6`/`libxrender1` — even after fix (1), the container still crashed with `ImportError: libxcb.so.1`, since `ultralytics` pulls in the full GUI-capable `opencv-python`, not headless, which needs system libraries `python:3.10-slim` doesn't ship; `.env` now loaded via `env_file`; `frontend` service added to Compose, satisfying FR-DK.2's "frontend, backend, auditor/batch service" requirement in full. Image rebuild with fix (2) succeeded, but Docker Desktop became unresponsive before the container could be started and `/health` re-confirmed — treat as applied-not-yet-verified until that check lands) |
...
| FR-CV.1–3, FR-CV.8–9 | Crop classifier (multi-label R09/R04/R17/R10/R28/R08) | Stretch | §4.4 | Noor | **Done** — backbone changed to `swin_tiny_patch4_window7_224` after a 33-model comparison sweep (`docs/crop_classifier_comparison_findings.md`), best val macro-F1 0.274 (epoch 8), Rico holdout macro-F1 0.22 (best in sweep); wired into backend — `confirm_violations()` attaches `cv_confidence` to R08/R17/R04 after `check_rules()` |
| FR-CV.4–7 | Screenshot-only YOLO UI-detector fallback | Stretch | §4.4 | Noor (+ Salar notebook/train/eval) | **Done** — wired into backend; `xml` optional on `POST /api/v1/audit`, `_run_pipeline` falls back to `detect_ui()` → `detections_to_components()` on missing/malformed/empty-hierarchy XML; full 60-epoch MASC run (mAP50-95 0.322); Rico holdout mAP50 0.0258 → 0.2546 |
...
|----|-------------|---------------------|
| TBD-01 | Final LLM model / provider (cost vs quality) | **Resolved — Groq (llama-3.3-70b-versatile), provisional** (July 2026; see `docs/TBD-01-decision.md`) |
