...
Deterministic rules implemented first from parsed XML/components. R9 and R10 are optional enhancements.

| Rule ID | Issue | Detection logic | Severity |
|---------|-------|-----------------|----------|
| R1 | Missing accessible label | Clickable element has empty `text` and empty `content-desc`. | High |
| R2 | Image button without description | `ImageButton` / `ImageView` is clickable but `content-desc` is missing. | High |
| R3 | Duplicate labels | Multiple clickable elements have the same visible `text` or `content-desc`. | Medium |
| R4 | Small touch target | Clickable element width or height is below the expected threshold (preferably 48dp). | Medium / High |
| R5 | Unlabeled input field | `EditText` has no hint, no `text`, and no `content-desc`. | High |
| R6 | Disabled important control | Important clickable/control element is disabled or inaccessible without a clear reason. | Medium |
| R7 | Invisible or zero-size component | Element has invalid `bounds` or zero visible area. | Medium |
| R8 | Possible layout overlap | Two important UI elements have overlapping `bounds`. | Medium |
| R9 | Low contrast *(optional)* | Estimate foreground/background contrast from screenshot crop. | Medium |
| R10 | Text overflow *(optional)* | Text component `bounds` are too small for visible text or likely clipped. | Low / Medium |
...
| Rule ID | Issue | Detection logic | Severity |
|---------|-------|-----------------|----------|
| R11 | No color-independent indicator | UI uses color as the only way to convey information (e.g. red = error) with no icon, text, or pattern alongside it. | High |
| R12 | No visual focus indicator | Focusable element has no visible focus ring or highlight state defined (missing `android:state_focused` drawable). | High |
| R13 | Color contrast for non-text elements | Interactive elements (buttons, icons, borders) have contrast ratio below 3:1 against adjacent colors. | Medium |
| R14 | Missing captions or transcript indicator | Media element (video/audio player) detected with no visible caption toggle, subtitle button, or transcript link nearby. | High |
| R15 | Audio-only alert or notification | Alert or notification relies solely on sound with no visible banner, badge, or vibration indicator attribute present. | High |
| R16 | Insufficient touch target spacing | Two or more clickable elements placed with a gap less than 8dp, risking accidental activation. | Medium |
| R17 | Auto-playing media without pause control | Media element auto-starts with no accessible pause/stop button reachable within first focusable elements. | Medium |
| R18 | Flashing or animated content | Element suggests rapid animation or flashing (`resource-id`/`class` contains `flash`, `blink`, `strobe`) that may trigger photosensitive seizures. | Critical |
| R19 | Form field with no programmatic label association | `EditText` exists alongside a `TextView` label but not programmatically linked (no `labelFor` attribute set). | High |
| R20 | Redundant or meaningless content description | Element's `content-desc` contains generic text like `image`, `icon`, or `button` rather than describing the actual action or meaning. | Medium |
| R21 | Missing error suggestion on input field | `EditText` that has a validation error but no `errorMessage` or hint describing what valid input looks like. | Medium |
| R22 | Scrollable content with no scroll announcement | `ScrollView` or `RecyclerView` present but has no `contentDescription` indicating scrollable region for screen reader users. | Medium |
| R23 | Language not set on text content | App-level or element-level locale/language attribute is missing, preventing screen readers from reading text in the correct language. | Medium |
| R24 | Timed content with no extension option | Screen contains a countdown or session timer with no detectable option to extend or disable the time limit. | High |
| R25 | Missing skip navigation option | Long repeated navigation blocks (e.g. sidebars, menus) have no skip link or shortcut to jump to main content. | Medium |
| R26 | Interactive element not announced as interactive | A clickable element has no role or class indicating it is a button/control, so screen readers treat it as plain text. | High |
| R27 | Modal or dialog not trapping focus | Dialog/popup is present but focus is not confined within it, allowing screen reader to navigate behind the overlay. | High |
| R28 | Missing loading or progress state announcement | Progress bar or loading spinner has no `contentDescription` or live region attribute to announce state to screen readers. | Medium |
| R29 | Icon-only button with no label | Button contains only an icon drawable with no text and no `content-desc`, giving screen reader users no context. | High |
| R30 | Placeholder text used as only label | `EditText` relies solely on `android:hint` as its label with no separate visible or programmatic label linked to it. | Medium |
...
| Motor / physical | R4, R16, R24 |
| Screen reader / blind | R1, R2, R5, R12, R19, R20, R21, R22, R25, R26, R27, R28, R29, R30 |
| Cognitive | R21, R23, R25, R30 |
