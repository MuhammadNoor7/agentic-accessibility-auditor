...
**Practical work completed — Weeks 1–8 + post–Week 7 YOLO track (baseline 1 July 2026; updated 30 July 2026)**  
...
| Team | Salar (parser, MASC data, rules, tests, YOLO notebook, full YOLO train + Rico eval); Ayesha (Rico holdout, frontend, SRS, rules, prompt experiments); Noor (backend, JSON schemas, SDS, agent, reports, rules, SRS review, tests, QA, Colab YOLO training, crop classifier + Rico eval) |
...
| Report version | **v1.26** (30 Jul 2026) |
...
4. **Wire screenshot-only fallback into `src/agent.py` / audit path when XML is absent** — still open, **deliberately deferred**: SRS/SDS classify this as a Stretch requirement, and the team decided to prioritize other work first. Not a blocker for the core (XML-based) pipeline, which is fully independent of this module.
...
| Inference module | `src/crop_violation_classifier.py` (`classify_crop()`, `confirm_violations()`) | Present, imports cleanly, **not called from `backend/` or `src/agent.py` yet** |
...
**Not yet done:** wiring `classify_crop()`/`confirm_violations()` into the main audit pipeline. The natural integration point is running it on R09/R04/R17/R10/R28/R08 candidate regions after the XML rule check, to confirm or downgrade violations the XML-only pass can't fully verify visually — same open item as the YOLO detector (§15C.1). This is a deliberate, deferred decision (both models are Stretch requirements per SRS §4.4), not an unfinished blocker.
...
**Classification report (Rico holdout, checkpoint never retrained):**
...
|---|---:|---:|---:|
| micro avg F1 | 0.63 | 0.42 | -0.21 |
| macro avg F1 | 0.25 | 0.18 | -0.07 |
| R08 F1 | 0.70 | 0.46 | -0.24 |
| R17 F1 | 0.47 | 0.35 | -0.12 |
| R04 F1 | 0.36 | 0.25 | -0.11 |

**Reading the result:** every rule's F1 drops on Rico vs. MASC, confirming the model is not purely memorizing MASC's specific visual style (a total collapse to ~0 would indicate pure memorization) but does show meaningful generalization loss on unseen apps — expected given the small (25K-crop) training set and MASC-only training (per SRS FR-CV.5/BR-4/BR-5, Rico is reserved strictly for eval). R08 (layout overlap) remains the strongest-performing rule on both datasets, consistent with it being the highest-volume, best-represented class throughout.

**Sample Rico predictions (true label vs. model prediction, 12 crops the model never saw in training or MASC evaluation):**

![Sample Rico predictions](assets/crop_classifier/sample_rico_predictions.png)
...
├── crops/
│   ├── train/                 ← 5,516 PNG files (partial local sample of the 25,202-row train manifest)
...
├── manifests/
│   ├── train.csv                ← 25,202 rows (labels for training; not all backed by a local PNG, see below)
│   ├── val.csv                  ← 5,558 rows (labels only — no local val/ crop images saved)
│   └── test.csv                 ← 5,541 rows
├── rico_manifest/
│   └── holdout.csv              ← 9,343 rows (Rico holdout labels, §15G.7)
└── export/
    └── crop_violation_classifier_best.pt   ← 6.2 MB, epoch-17 checkpoint (also mirrored to models/)
```

**Physical crop images vs. manifest rows — known gap:**

| Split | Manifest rows | Physical PNG files on disk | Coverage |
|---|---:|---:|---:|
| train | 25,202 | 5,516 | ~22% (partial local sample) |
| val | 5,558 | 0 | 0% (labels only, no images retained locally) |
| test | 5,541 | 5,541 | 100% |
| Rico holdout | 9,343 | 9,343 | 100% |

This is a disk-space artifact of the Colab workflow, not a data-quality problem: the manifest CSVs (which drive training/eval) are complete for every split, and the two splits actually needed for the evidence in this report — the MASC test-set classification (§15G.5) and the Rico holdout eval (§15G.7) — have their crop images fully present locally. The `train`/`val` image folders were not fully synced back from Colab since they aren't needed to reproduce evaluation, only retraining.

**Sizes on disk:** `crops/train/` 110 MB · `crops/test/` 110 MB · `rico_crops/holdout/` 177 MB · `manifests/` 3.7 MB · `rico_manifest/` 956 KB · `export/` 6.0 MB — **~407 MB total**, all tracked in git except the `.pt` checkpoint (`runs/**/*.pt` is gitignored per policy; mirrored separately to `models/crop_violation_classifier_best.pt`, also gitignored).
...
|------|----------|--------------------|-----------------|
| **YOLO UI-element detector integration** | §1.4: "In progress... not yet wired into pipeline" | `src/yolo_ui_detector.py` **exists** (ported from Salar's `salar` branch, 30 Jul). Full 60-epoch checkpoint (51.2 MB) at `models/yolo_ui_detector_best.pt`; Rico zero-shot-vs-fine-tuned eval done (§15C.9: mAP50 0.0258 → 0.2546). Nothing in `backend/` or `src/agent.py` calls the model | Wire `detect_ui()` as the screenshot-only fallback path per SRS FR-CV.4–7 — **deliberately deferred** (Stretch requirement), not started |
| **Crop-violation classifier integration** | Not in SDS yet (built after v2.12) | Trained, checkpoint + inference module exist (§15G.6); Rico holdout eval done (§15G.7: weighted F1 0.43); not called from anywhere in the pipeline | Decide the integration point (post-XML-check pixel confirmation) and wire it — **deliberately deferred** (Stretch requirement), not started |
...
> Machine log copy: `docs/progress/assets/logs/noor_week8_validation_log.txt`
