export default function GoogleButton({ label = 'Continue with Google', onClick }) {
  return (
    <button
      type="button"
      onClick={onClick}
      className="w-full min-h-[48px] flex items-center justify-center gap-3 rounded-lg border border-[var(--color-border)]
        bg-white text-[15px] font-medium text-[var(--color-navy)]
        hover:bg-[var(--color-page-bg)] transition-colors duration-150
