...
| pytest suite | `tests/`, `backend/tests/` | Salar / Noor | **Done** (**152** collected; auth 22, rules 82, parser 12, audit 9, …) |
| YOLO UI detector (screenshot-only fallback) | `src/yolo_ui_detector.py`, `notebooks/train_yolo_ui_detector.ipynb`, `runs/` | Salar (notebook, full train, Rico eval) / Noor (Colab train, merge) | **Trained + Rico-evaluated** — full 60-epoch run (mAP50-95 0.322); `best.pt` exported (51.2 MB); not wired into pipeline (deliberately deferred) |
| Crop-violation classifier (pixel-level rule confirmation) | `src/crop_violation_classifier.py`, `notebooks/train_crop_violation_classifier.ipynb`, `runs/crop_violation_classifier/` | Noor (Colab train + Rico eval) | **Trained + Rico-evaluated** — 20 epochs, best val macro-F1 0.265; MASC test weighted F1 0.63, Rico holdout weighted F1 0.42; `crop_violation_classifier_best.pt` exported (6.2 MB); not wired into pipeline (deliberately deferred) |
...
**SRS:** FR-CV.4–FR-CV.7 | **Owner:** Salar (training notebook, full 60-epoch train, Rico eval) / Noor (Colab training, merge, inference module) | **Status:** Trained (full 60-epoch run) + Rico-evaluated — inference module exists and works, not pipeline-wired (deliberately deferred, Stretch requirement)
...
| Design point | Detail |
|---------------|--------|
| Output shape | List of per-detection dicts (bbox, class, confidence) — matching §4.2 `components.json` semantics is still a wiring task, not yet mapped |
| Confidence | Default `conf=0.25` threshold, configurable per call |
| Integration point (planned) | `src/agent.py` / `backend/routers/audit.py` — if XML parsing fails or is absent, fall back to `detect_ui(screenshot)` instead of `build_screen_document(...)` before handing off to the rule engine (§3.2) |
...
**Not yet done:** pipeline wiring (fallback trigger in `src/agent.py`/`backend/routers/audit.py`, which currently hard-requires XML). This is a deliberate, deferred decision — both CV modules remain Stretch requirements and the team has chosen to prioritize other work first, not an unfinished blocker on the core pipeline. Tracked in Progress Report §15C.1 / §15C.6.

### 3.7 Crop-violation classifier module (pixel-level rule confirmation)

**SRS:** FR-CV.1–FR-CV.3 (supplementary CV signal, R09 contrast, never silently override rule results) | **Owner:** Noor (Colab training + inference module) | **Status:** Trained + Rico-evaluated, inference module working, **not pipeline-wired** (deliberately deferred, Stretch requirement)
...
**Rico holdout generalization eval (30 Jul 2026):** same checkpoint, no retraining, run against 9,343 crops built from all 1,698 Rico holdout screens (never used in training):
...
Every rule's F1 drops vs. the MASC test split (R08: 0.70 → 0.46; R17: 0.47 → 0.35; R04: 0.36 → 0.25) — expected given the small (25K-crop), MASC-only training set, and consistent with genuine generalization loss rather than pure memorization (a collapse to ~0 would indicate the latter). R08 remains the strongest rule on both datasets. Full breakdown and sample predictions: Progress Report §15G.7.

#### 3.7.5 Inference module design

```python
# src/crop_violation_classifier.py (real — imports and runs; same status as §3.6.4's
# src/yolo_ui_detector.py, which also now exists and works)
def classify_crop(
    image_path: str | Path,
...
**Not yet done:** pipeline wiring (nothing in `backend/` or `src/agent.py` calls `classify_crop()` yet — same open item as §3.6's YOLO detector), and no automated tests exist for this module. Both CV modules are deliberately deferred (Stretch requirements per SRS §4.4), not unfinished blockers. Tracked in Progress Report §15G.6–§15G.7.
...
| FR-CV.4–7 | §3.6 | `src/yolo_ui_detector.py`, `notebooks/train_yolo_ui_detector.ipynb` (**Trained + Rico-evaluated, not wired**) |
...
| `src/yolo_ui_detector.py` | Screenshot-only YOLO fallback inference (**Trained + Rico-evaluated**; exists and works; not pipeline-wired, deliberately deferred) |
...
| `src/crop_violation_classifier.py` | Pixel-level crop classifier inference (`classify_crop()`, `confirm_violations()`) (**Trained + Rico-evaluated**; exists and works; not pipeline-wired, deliberately deferred) |
