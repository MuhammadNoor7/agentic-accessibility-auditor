...
**Current integration branch:** [`noor`](https://github.com/MuhammadNoor7/agentic-accessibility-auditor/tree/noor) — Weeks 1–6 complete + **Week 7 QA analysis** (Noor): rule/guideline summaries on MASC 40-screen sample; parser + **R01–R30**, agent + HTML/PDF export, Axion UI, **JWT + OTP/SMTP + Google OAuth**, validation logs; Rico holdout **deferred**. **Post–Week 7 (27 Jul):** screenshot-only **YOLO UI-element detector** track — MASC-only training run (mAP50-95 0.285), `best.pt` exported, `src/yolo_ui_detector.py` scaffolded (`runs/`).
...
| YOLO UI detector (post–Week 7) | Salar (notebook) / Noor (Colab train) | `best.pt`, `src/yolo_ui_detector.py` | **In progress** — MASC-only run, 1 epoch logged (mAP50-95 0.285); `runs/`; not pipeline-wired |
...
│   ├── llm_providers.py
│   ├── schema_documents.py
│   └── yolo_ui_detector.py  # screenshot-only YOLO fallback (in progress, not pipeline-wired)
│
...
└── runs/                   # YOLO UI detector track (post–Week 7)
    ├── notebooks/          # train_yolo_ui_detector.ipynb, masc_dataset_analysis.ipynb, base weights
    └── runs/yolo_ui_detector/
        ├── export/         # yolo_ui_detector_best.pt
        ├── yolo_dataset/, rico_yolo_dataset/  # MASC train vs Rico eval-only configs
        └── runs/yolo_ui_detector/             # args.yaml, results.csv, plots
```
